...
const ApproveClasses = ({ approvedClasses, isLoading }) => {
    return (
        <div>
            {approvedClasses.length > 0 ? <div>
                <div className="p-5">
                    {
                        !isLoading ? <>

                            <div className="overflow-x-auto bg-[#ffb277] p-5 rounded-lg">
                                <h1 className="mb-3 text-2xl font-semibold text-black">Approved Classes:</h1>
                                <table className="table border border-[white]">
                                    ...
                                    <tbody>
                                        {
                                            approvedClasses.map(c => <tr key={c._id}>
                                                <td>
                                                ...
                                                <th>
                                                    <div className="flex">
                                                        <button className="m-1 border-0 shadow-md p-2 rounded-lg text-white bg-success font-normal flex justify-center items-center gap-1" disabled={c.status === 'Approve' ? true : false}>
                                                            <FaRegCalendarCheck size={16} />
                                                            Approve
                                                        </button>
                                                        {/* Open the modal using ID.showModal() method */}
                                                        <button disabled={c.status === 'Approve' ? true : false} className="m-1 border-0 shadow-md p-2 rounded-lg text-white bg-error font-normal flex justify-center items-center gap-1" ><FaRegCalendarTimes size={16} />
                                                            Deny</button>
                                                    </div>
                                                </th>
                                            </tr>)
